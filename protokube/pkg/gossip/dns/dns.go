...
func RunDNSUpdates(target DNSTarget, src *DNSView) {
	...
	for {
		...
		//		if strings.HasPrefix(k, "dns/a/") {
		//			hostname := strings.TrimPrefix(k, "dns/a/")
		//			addresses := strings.Split(v, ",")
		//			hostToAddr[hostname] = append(hostToAddr[hostname], addresses...)
		//		} else {
		//			glog.Warningf("unknown tag for peer %q: %q=%q", peer.id, k, v)
		//		}
		//	}
		//}

		// TODO: Check if it has actually changed?
		//for k, v := range hostToAddr {
		//	if len(v) <= 1 {
		//		continue
		//	}
		//	sort.Strings(v)
...
func (g *DNSView) Snapshot() *DNSViewSnapshot {
	...
	//hostToAddr := make(map[string][]string)
	//for _, peer := range gossipSnapshot {
	//	if !peer.Healthy {
	//		// TODO: We might want to keep alive old records for a bit, in case serf fails but the machine hasn't
	//		// Also, we might want the concept of exclusive vs shared records
	//		continue
...
//// TODO: This is not the greatest layering
//func (g *GossipState) SetHostname(hostname string, values []string) error {
//	g.mutex.Lock()
//	defer g.mutex.Unlock()
//
//	key := "dns/a/" + hostname
//	g.tags[key] = strings.Join(values, ",")
//
//	if g.serf != nil {
//		err := g.serf.SetTags(g.tags)
//		if err != nil {
//			return fmt.Errorf("error broadcasting tags: %v", err)
//		}
//	}
//	return nil
//}
