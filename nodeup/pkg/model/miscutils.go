...
func (b *MiscUtilsBuilder) Build(c *fi.ModelBuilderContext) error {
	...
	case distros.DistributionContainerOS:
		glog.V(2).Infof("Detected ContainerOS; won't install misc. utils")
		return nil
	case distros.DistributionCoreOS:
		glog.V(2).Infof("Detected CoreOS; won't install misc. utils")
		return nil
	}
	...
	if b.Distribution.IsDebianFamily() {
		...
		glog.Warningf("unknown distribution, skipping misc utils install: %v", b.Distribution)
		return nil
	}

	switch b.Distribution {
	case distros.DistributionUbuntu:
