...
func RunGet(ctx context.Context, f commandutils.Factory, out io.Writer, options *GetOptions) error {
	...
	var additionalObjects []*unstructured.Unstructured
	{
		...
		for _, addon := range addons {
			additionalObjects = append(additionalObjects, addon.ToUnstructured())
		}
	}
	...
	if options.Output != OutputTable {
		...
		for _, additionalObject := range additionalObjects {
	...
	case OutputTable:
		...
		if len(additionalObjects) != 0 {
			fmt.Fprintf(out, "\nAdditional Objects\n")
			err = addonsOutputTable(cluster, additionalObjects, out)
			if err != nil {
				return err
			}
		}
