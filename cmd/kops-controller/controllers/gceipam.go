...
// GCEIPAMReconciler observes Node objects, TODO
...
func (r *GCEIPAMReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if len(node.Spec.PodCIDRs) == 0 {
		...
		if err != nil {
			return ctrl.Result{}, fmt.Errorf("error parsing providerID %q: %w", node.Spec.ProviderID, err)
		}
		...
		if err != nil {
			return ctrl.Result{}, fmt.Errorf("error getting instance %s/%s/%s: %w", project, zone, instanceID, err)
		}

		if len(instance.NetworkInterfaces) != 1 {
			return ctrl.Result{}, fmt.Errorf("unexpected number of network interfaces for instance %q: %v", instanceID, len(instance.NetworkInterfaces))
		}

		var ipv6Addresses []string
		for _, ipv6AccessConfig := range instance.NetworkInterfaces[0].Ipv6AccessConfigs {
		...
		if err := patchNodePodCIDRs(r.coreV1Client, ctx, node, ipv6Address); err != nil {
			return ctrl.Result{}, fmt.Errorf("error patching node: %w", err)
		}
	}
	...
	if len(node.Spec.PodCIDRs) != 0 {
		...
		if allIPv6 && len(node.Spec.PodCIDRs) != 0 {
				...
				if condition.Type == "NetworkUnavailable" {
					if condition.Status == corev1.ConditionTrue {
						...
						if err := patchStatusCondition(ctx, r.client, node, r.fieldOwner, newCondition); err != nil {
							return ctrl.Result{}, fmt.Errorf("error updating NetworkUnavailable condition: %w", err)
						}
					}
				}
			}
...
func patchStatusCondition(ctx context.Context, kube client.Client, obj client.Object, fieldOwner string, condition metav1.Condition) error {
	...
	if err := kube.Status().Patch(ctx, obj, client.RawPatch(types.ApplyPatchType, patchJSON), client.ForceOwnership, client.FieldOwner(fieldOwner)); err != nil {
		return fmt.Errorf("error applying patch to %v %v: %w", kind, obj.GetName(), err)
	}
