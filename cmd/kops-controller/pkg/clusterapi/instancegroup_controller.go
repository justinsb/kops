...
func (r *InstanceGroupReconciler) updateObject(ctx context.Context, obj *unstructured.Unstructured) error {
	...
	if err != nil {
		if apierrors.IsNotFound(err) {
			...
			return fmt.Errorf("error getting %s %s/%s: %w", gvk.Kind, namespace, name, err)
		}
	}
	if existing == nil {
		var opts metav1.CreateOptions
		if _, err := res.Create(ctx, obj, opts); err != nil {
			return fmt.Errorf("error creating %s %s/%s: %w", gvk.Kind, namespace, name, err)
		}
		return nil
	}
